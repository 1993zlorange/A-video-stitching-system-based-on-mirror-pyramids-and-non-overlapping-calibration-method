...
void imgout2imgin_cor::errect2fisheye(float yaw, float pitch, float roll, float x_src, float y_src, int w_src, int  h_src, int w_dst, int h_dst, float kk_dst[3][3], float D_dst[4], float xy_dst[2])
{
	float cor_wout[3];
	float cor_win[3];
	...
	float yw = cor_win[1];
	float zw = cor_win[2];
	w2i.world2fisheye(xw, yw, zw, D_dst, kk_dst, h_dst, w_dst, xy_dst);

}

void imgout2imgin_cor::fisheye2erect(float yaw, float pitch, float roll, float x_src, float y_src, int w_src, int w_dst, int h_dst, float fov_src, float xy_dst[2])
{
	float cor_wout[3];
	float cor_win[3];

	i2w.fisheye2world(x_src, y_src, fov_src, w_src, cor_wout);
	float x_wout = cor_wout[0];
	float y_wout = cor_wout[1];
	float z_wout = cor_wout[2];
