...
void wrd2img::world2fisheye(float x_src, float y_src, float z_src, float D_dst[4], float kk[3][3], int h_dst, int w_dst, float xy_dst[2])
{
	//float f = sqrt(w_dst*w_dst + h_dst*h_dst) / fov_dst;
	float fov_dst = sqrt(w_dst*w_dst + h_dst*h_dst) / kk[0][0];
	float r = sqrt(x_src*x_src + y_src*y_src);
	if (z_src > 0)
	{
		...
		xy_dst[0] = kk[0][0] * x_src + kk[0][2];
		xy_dst[1] = kk[1][1] * y_src + kk[1][2];
	}
	else
	{
		xy_dst[0] = -1;
