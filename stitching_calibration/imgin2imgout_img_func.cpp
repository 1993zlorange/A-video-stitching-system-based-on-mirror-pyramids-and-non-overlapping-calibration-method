...
//
//			float cor_image[2] = { 0, 0 };
//			imgotut2imgin_T.errect2perspective(yaw, pitch, roll, i, j, w_dst, h_dst, col, row, fov_dst, cor_image);
//			u = cor_image[0];
//			v = cor_image[1];
//
//			if (floor(u) >= col || floor(v) >= row || floor(u) < 0 || floor(v) < 0)
...
void imgin2imgout_img::fisheye2errect_map(Mat SrcImage, float yaw, float pitch, float roll, float D_dst[4], int w_dst, int h_dst, float kk_dst[3][3], Mat m_maps_x, Mat  m_maps_y, float corner[2], float m_size[2])
{
	int row = SrcImage.rows;
	int col = SrcImage.cols;
	int x_min = row;
	...
	for (int i = 0; i <w_dst; i = i + 1)
	{
		for (int j = 0; j <h_dst; j++)
		{
			...
			float cor_image[2] = { 0, 0 };
			imgotut2imgin.errect2fisheye(yaw, pitch, roll, i, j, w_dst, h_dst, col, row, kk_dst, D_dst, cor_image);
			u = cor_image[0];
			v = cor_image[1];
...
Mat imgin2imgout_img::erect2fisheye_image(Mat SrcImage, float yaw, float pitch, float roll, int w_dst, int h_dst, float fov_dst)
{
	Mat DestData = Mat(cvSize(w_dst, h_dst), CV_8UC3);
	int row = SrcImage.rows;
	int col = SrcImage.cols;
	for (int i = 0; i <w_dst; i = i + 1)//每一列扫描
	{
		for (int j = 0; j <h_dst; j++)
		{
			...
			float cor_image[2] = { 0, 0 };
			imgotut2imgin.fisheye2erect(yaw, pitch, roll, i, j, w_dst,  col, row, fov_dst, cor_image);
			u = cor_image[0];
			v = cor_image[1];
			...
			else
			{
				double a = 0;
				//DestData.at<Vec3b>(j, i) = a;
				DestData.at<Vec3b>(j, i) = DestData.at<Vec3b>(j-1, i);
			}
		}
	}
